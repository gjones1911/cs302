...
class Superball {
	public:
		...
		set <int> goals_mp;	//will hold the coordinates to all goals for distance checking/
...
//	int co = 0;
		...
		if(mvslft >= 5 ) 
		{
			...
			if(mvslft >= 7)
			{
				printf("SWAP %d %d %d %d\n",hg_rnk.r1, hg_rnk.c1,hg_rnk.r2, hg_rnk.c2 );
			}
			else 
			{
		...
		for (j = 0; j < c; j++) 
		{
			...
			vh[1] = 1;					//set this set's size to 1
				...
				else if(board[i*c+j] != '.')
				{
					...
					vh2.push_back(board[i*c + j] );		//store the value of the char in this square
					vh2.push_back(i);					//store the row 
					vh2.push_back(j);					//store the column
					
					non_goal_rc.push_back(vh2);			//save non goal "color" and coordinates 
...
void Superball::analyze_superballB()
{
	...
	//			printf("Size: %2d Char: %c Scoring Cell: %d,%d\n",mit->second[1], board[mit->first], mit->second[2], mit->second[3] ) ;
				...
				//get the colorts and coordinates of the goals
				goals_mp.insert(board[mit->first] );  
				...
				if(scm.points < mit->second[1] * colors[ board[mit->first] ] )
				{
					...
					scm.scores.clear();
					scm.scores.push_back( mit->second[2]   ) ; //save the row of this score block
					scm.scores.push_back( mit->second[3]  ) ;  //save the column of this score block
	...
	int mxg = 0;
	int idx = 0;
	//find max non goal colored square
	...
	//if you have less than or equal to  20 colored squares move higher valued blocks into goals spots
	if(r*c - empty <= 15)
	{
...
//				printf("the non goal block is: %4c \n",non_goal_rc[j][0]);
				...
				{
					//need to look at the goal im trying to replace and compare the points lost for the move
...
void Superball::analyze_superball(int r1, int c1, int r2, int c2)
{
	...
	vector<int> clr_cnt(256,0);		//will be used to count the number of blocks of each color on the board
	...
	for(mit = set_mp.begin(); mit != set_mp.end() ; mit++)
	{
		clr_cnt[board[mit->first]] += mit->second[1];;
		...
		if(board[mit->first] != '.' && board[mit->first]  != '*' && goals[mit->first] )
		{
			...
			if(mit->second[1] >= mss )
			{
				...
				mvpts+= mit->second[1] * colors[board[mit->first]];		//store the scoring moves total points
				scr_cnt++;

				scmsz += mit->second[1];	//store the total size of scoring moves
			}
			else		//if it is a goal but doesnt meet the minimun count it for the number of near  
			{
				...
				if( mit->second[1] >= mss - 2) 
				{
					//= number of blocks * block value finds near scoring points
					nrpts += mit->second[1] * colors[board[mit->first]];
					a_scr_cnt++;
...
int clrr;
	...
	//try look at the most prominent color on the board and compare it to the goal colors it it matches give this board a lot of pt
	for(i = 0; i < clr_cnt.size();i++)
	{
		if(clr_cnt[i] != 0)
		{
			if(most_p < clr_cnt[i])
			{
				most_p = clr_cnt[i];			//i is the color value or char value
				clrr = i;
			}
	...
	for(sit = goals_mp.begin(); sit != goals_mp.end();sit++)
	{
		//if the largest number of blocks corresponds
		//to a goal give this board a lot of points
		if(*sit == clrr && colors[clrr] > 0) 
		{
			isit = true;

			mst_gl = most_p * colors[clrr];
		}
	}
	...
	//now rank board
//	rank = mvpts + nrpts + gscore + score + mxst;
	rank = mvpts + nrpts +  mst_gl;

	if(rank > hg_rnk.points)
	{
		hg_rnk.size = rank;
		hg_rnk.points = rank;
		hg_rnk.r1 = r1;
