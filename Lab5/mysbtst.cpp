...
int cll=0;
	...
	for(mit = set_mp.begin(); mit != set_mp.end() ; mit++)
	{
		// board points + = size of this set * its point value(colors[board[id]])
		brdpts += mit->second[1] * colors[ board[mit->first] ];
		
		if(bgst < mit->second[1]) bgst = mit->second[1];
		...
		if(board[mit->first] != '.' && board[mit->first]  != '*' && goals[mit->first] )
		{
			...
			if(mit->second[1] >= mss )
			{
				...
				mvpts+= mit->second[1] * colors[board[mit->first]];
				scr_cnt++;
				if(em.size <= mit->second[1] )
				{
					em.size = mit->second[1];
					em.points = colors[board[mit->first] ] * mit->second[1] ;
					em.r1=r1;
					em.c1=c1;
					em.r2=r2;
					em.c2=c2;
		...
		else if(board[mit->first] != '.' && board[mit->first]  != '*' && !(goals[mit->first] ) )
		{
			...
			if(mxst <= mit->second[1] && colors[cll] < colors[board[mit->first] ] ) 
			{	
				mxst = mit->second[1];		//if its larger save it
				cll = board[mit->first];	
	...
	//now rank board
//	rank = mvpts + nrpts + gscore + score + mxst * cll;   //195
//	rank = mvpts + nrpts + mxst * cll;						//167

//	rank = mvpts + nrpts + gscore ;							//105
	
	
//	rank = mvpts + nrpts + gscore + score + mxst ;			//168 
	
//	rank = mvpts + gscore + score + mxst * cll;				//135/
//	rank = mvpts + gscore + mxst * cll;				//180/
//	rank = mvpts + mxst * cll;				//131/
	

//	rank = nrpts + gscore + mxst*cll;		///206
//	rank = nrpts + gscore + score;;		//218/
//	rank = nrpts + gscore ;		//105/
//	rank = score + gscore + mxst*cll;		//135/
//	rank = score + nrpts + mxst*cll;		//147/
//	rank = score + nrpts;		//169/
//	rank = nrpts;		//47/
//	rank = score;		//191/
//	rank = mvpts ;				//0/
//	rank = mxst * cll;				//127//
...
//	rank = mxst;		//140/
//	rank = mxst + score; //222
//	rank = mxst + score*10 + nrpts ;		//242
//	rank = scr_cnt * mvpts + score ;		//286
//	rank = scr_cnt * mvpts + score + mxst;		//305

	
//**************	
////	rank = scr_cnt * mvpts + score + mxst;		//345
//**************

//		rank = score + mvpts + mxst + scr_cnt;
		rank = brdpts;								//38
		...
		rank = score + nrpts + bgst*mvpts + gscore + mxst;			//245
		
		rank = score * nrpts + bgst*mvpts + gscore * mxst;			//
		rank = score + bgst + nrpts;			//
