...
//					set_mp.erase(sidup);
		...
		look_bk(board[sidup], sidup ,nwsid ,i-1 ,j);
...
//					set_mp.erase(siddwn);
		...
		look_bk(board[dwnid], dwnid ,nwsid ,i ,j-1);
		}
...
//	prnt_mp(set_mp,0);
	...
	for(mit = set_mp.begin(); mit != set_mp.end() ; mit++)
	{
		...
		look_bk(color,myid,sid,row,col);
	}
