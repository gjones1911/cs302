...
int main(int argc, char **argv)
{
	...
	map <string, Artist *> ::iterator t_it;
	...
	for( t_it = table.begin(); t_it != table.end(); t_it++)
	{
		...
		//print in the artist info in the form: 
		//										artist name: song count, Min:sec
		...
		for(alb_it = artist->albums.begin(); alb_it != artist->albums.end();alb_it++ )
		{
			...
			i = album->scount;
			//use the pointer to get the album info
			...
			if(minsec.at(1) < 10)
			{
				...
				cout<<"        "<<shldr<<": "<<i<<", "<<minsec.at(0)<<":0"<<minsec.at(1)<<"\n";			
			}
			else
			{
				cout<<"        "<<shldr<<": "<<i<<", "<<minsec.at(0)<<":"<<minsec.at(1)<<"\n";			
			}
//			printf("%10s: %d, %d:%d\n",shldr.c_str(), i,minsec.at(0), minsec.at(1) );
			...
			for(crnt_sng = album->songs.begin(); crnt_sng != album->songs.end(); crnt_sng++)
			{
				...
				if(minsec.at(1) < 10)
				{
					...
					cout<<"                "<<song->track_number<<". "<<song->name<<": "<<minsec.at(0)<<":0"<<minsec.at(1)<<"\n";
//					printf("%16d. %s: %d:0%d\n",song->track_number, song->name.c_str(), minsec.at(0), minsec.at(1));
				}
				else
				{
					cout<<"                "<<song->track_number<<". "<<song->name<<": "<<minsec.at(0)<<":"<<minsec.at(1)<<"\n";
