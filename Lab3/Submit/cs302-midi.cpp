...
void CS302_Midi::el_to_nd()
{
	...
	bool dmp_dn = false;            //will be used to determine if the damper pedal is down
	
//	bool note_on = false;

	int note_cnt = 0;				//will be used to make sure there are an even number of on and off events
	int dmp_cnt = 0;				//will be used to make sure there are an even number of up and down events
	...
	for(e_it = (*el).begin(); e_it != (*el).end(); e_it++)
	{
		...
		if(type == 'O' || type == 'F')
		{
			...
			if(type == 'O')
			{
				...
				{
					cerr<<"There is a on note for pitch "<<pitch<<" that is being turned on when it is already on!\n";
					exit(1);
				}
			...
			else	//its the next off event off set its off time and store it in the NDMap
			{
				if(npvec.at(pitch) == NULL)
				{
					cerr<<"there is a missing on note for: "<<pitch<<"\n";
					exit(1);

				}

				//set the off time and store in NDmap
				...
				//set this index back to null to represent the note is off
				npvec.at(pitch) = NULL;
		...
		else if(type == 'D')		//its a damper event
		{
			...
			if(ud)
			{
				//check for a damper down event following another damper down event
				if(dmp_dn == true)
				{
					cerr<<"a damper down event is occuring after another damper down event\n";
					exit(0);
				}
	...
	//check for a on note with no off time or a damper down event that never gets an up
	if(note_cnt > 0)
	{
		cerr<<"There is a missing off note!\n";
		exit(1);
	}

	if(dmp_cnt  > 0)
	{
		cerr<<"There is a missing damper up event!\n";
		exit(1);
	}
