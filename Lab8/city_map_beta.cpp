...
double City_Map::Dijkstra(int avg_best_worst)
{
	...
	while( !(bfsq.empty()) )
	{
		...
		ip->bfsq_ptr = bfsq.end();				//tell yourself you removed it
		...
		for(lit = ip->adj.begin(); lit != ip->adj.end(); lit++)
		{
			...
			mmit = bfsq.find(ip2->best_time);   //look for the kid in the map		

			ntm = ( (*lit)->distance /(30) ) * 3600 + ip->best_time;	//get the time it takes to run this segment

			//check that the kid hasnt been visited
			if(ip2->bfsq_ptr != bfsq.end() )
			{
...
//					ip2->backedge = rsp;
				...
				//otherwise check for it being in the map and if so compar the new time and the old time
				//if the old time is greater replace it with the shorter time
				else 
				{
					//if it is in the map remove it
					if(ip2->bfsq_ptr->second == ip2)
					{
						bfsq.erase( ip2->bfsq_ptr );
					}

					if(ip2->best_time > ntm )
					{

						ip2->best_time = ntm;					//store the new best time
						ip2->backedge = (*lit);
	
	/*
						ip2->backedge->from = ip;					//store the new backedge
						ip2->backedge->to = ip2;
						ip2->backedge->distance = (*lit)->distance;//store the new backedge
*/						
