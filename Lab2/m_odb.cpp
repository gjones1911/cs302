...
int main(int argc, char **argv)
{
	...
	if(argc > 1)
	{
		...
		if(cmd == "1")
		{
			while(getline(cin,shld) )
			{
				linevec.push_back( stovec( shld ) );	
			}

			for(i = 0; i < 2; i++)
			{
				for(j = 0; j < linevec.at(i).size() ; j++)
				{
					if(i == 0)
					{

						fp.push_back(  atoi( linevec.at(0).at(j).c_str() ) ) ;		
					}
					else
					{
						sp.push_back(  atoi( linevec.at(i).at(j).c_str() ) );
					}
				}
			}
	...
	printf("The number of guesses is: %lld\n",num);
