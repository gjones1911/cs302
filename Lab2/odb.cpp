...
long long OneDimensionalBalls::cvg( int &velocity)
{
	...
	while( !(done) )
	{
		//look through FP for unconnected balls
		for(i = 0; i < FP_ptr.size(); i++)
		{
			FBP = FP_ptr.at(i);

			// if a ball with no pointers is found end the run and return 0, no guesses can be made
			if( (FBP->low == NULL && FBP->high == NULL) || FBP->low->val == -1 && FBP->high->val == -1  )
			{
				//				cout<<"no guesses can be made\n";

				return 0;
			}
			//look for balls with only one pointer or bottom ball and remove
			else if(  (FBP->low != NULL && FBP->high == NULL) || (FBP->low == NULL && FBP->high != NULL) )
			{
				FBP->val = -1;

				//check if the bottom balls h/l has no other bottom balls if so no guess can be made 
				//due to a floating ball being created
				...
				if(FBP->low == NULL)
				{
					...
					if(HBP->high != NULL)
					{
						if(HBP->high->high == NULL) return 0;
						else
						{
							HBP->val = -1;
							HBP->low = NULL;	//disconnect the high ball from current FBP
							HBP->high->low = NULL; //disconnect the hb's high from it
							HBP->high = NULL       //disonnect the hb from its high
						    FBP->high = NULL;      //disconnect the FBP from the current hb		
						}
					}
					else if(HBP->high == NULL)
					{
							HBP->val = -1;
							HBP->low = NULL;	//disconnect the high ball from current FBP
						    FBP->high = NULL;      //disconnect the FBP from the current hb		
				...
				else if(FBP->high == NULL)
				{
					...
					if(LBP->low != NULL)
					{
						if(LBP->low->low == NULL) return 0;
						else
						{
							LBP->val = -1;
							LBP->low = NULL;	//disconnect the low ball from current FBP
							LBP->low->low = NULL; //disconnect the lb's high from it
							HBP->high = NULL       //disonnect the hb from its high
						    FBP->high = NULL;      //disconnect the FBP from the current hb		
						}
					}
		...
		for(i = 0; i < FP_ptr.size(); i++)
		{
			...
			{
				if(FP_ptr.at(i)->low != NULL)
				{
					if(FP_ptr.at(i)->low->val == -1)
					{
						if(FP_ptr.at(i)->high == NULL)
						{
							FP_ptr.at(i)->val = -1;

							done = false;
						}
						else if(FP_ptr.at(i)->high->val == -1)
						{
							FP_ptr.at(i)->val = -1;
							done = false;
						}
				...
				else if(FP_ptr.at(i)->low == NULL)
				{
					if(FP_ptr.at(i)->high != NULL)
					{
						if(FP_ptr.at(i)->high->val == -1)
						{
