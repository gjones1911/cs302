...
class OneDimensionalBalls
{
	public:
		...
		void set_balls(vector<int> &fp,vector<int> &sp);										//will set up the F/Sp_ptr vectors
		...
		vector <int> get_vs(vector <int> fp, vector <int> sp);								//will calculate the possible vectors for guessing
...
vector <int> get_vs()								//will calculate the possible vectors for guessing
{
	...
	vector <int>  diff;							//will hold the difference values for FP-SP
...
void OndDimensionalBalls::set_fpsp(vector <int> &fp, vector <int> &sp)
{
	fp0 = fp;
	Sp0 = sp;
}
